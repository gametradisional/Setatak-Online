...
function generateLevelQuestion(extraQuestions) {
  // 30% peluang ambil soal dari bank soal
  if (Math.random() < 0.6 && extraQuestions && extraQuestions.length > 0) {
  ...
  while (true) {
    ...
    if (op2 === '÷') {
      if (op1 === '+' || op1 === '-') {
        y = randInt(1, 6);
        const divs = getDivisors(y + z).filter(d => d <= 30);
        if (divs.length === 0) continue;
        z = divs[randInt(0, divs.length - 1)] - y;
      } else {
        const temp = evalNoPrecedence(x, y, op1);
        const divs = getDivisors(Math.max(1, Math.abs(temp))).filter(d => d <= 30);
        if (divs.length === 0) continue;
        z = divs[randInt(0, divs.length - 1)];
      }
    }
    ...
    let result;
    try {
      result = evalNoPrecedence(evalNoPrecedence(x, y, op1), z, op2);
    } catch {
      continue;
    }
...
function generateLevel2Question() {
  return generateLevelQuestion(extraQuestions2);
}

function generateLevel3Question() {
  return generateLevelQuestion(extraQuestions3);
}

function generateLevel4Question() {
  return generateLevelQuestion(extraQuestions4);
}

function generateLevel5Question() {
  return generateLevelQuestion(extraQuestions5);
}
...
function showWinner(winner) {
  // Tambahkan filterClass ke ikon di modal kemenangan
  const winnerIcon = document.getElementById('winnerIcon');
  winnerIcon.textContent = winner.character.icon;
  winnerIcon.className = 'text-6xl mb-2'; // Hapus kelas filter
